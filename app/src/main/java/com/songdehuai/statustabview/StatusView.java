...
public class StatusView extends LinearLayout {
    ...
    public StatusView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int isLeftValues = ta.getInt(R.styleable.StatusView_retinue, 0);
        isLeft = (1 == isLeftValues);
        selectedColor = ta.getColor(R.styleable.StatusView_selectedColor, Color.WHITE);
        normalColor = ta.getColor(R.styleable.StatusView_normalColor, Color.WHITE);
        slashAngle = ta.getInteger(R.styleable.StatusView_slashAngle, 0);
        textColor = ta.getColor(R.styleable.StatusView_textColors, Color.GRAY);
        selectedTextColor = ta.getColor(R.styleable.StatusView_selectedTextColor, Color.GREEN);
        temp = ta.getString(R.styleable.StatusView_texts);
        texts = temp.split("\\|");
        ta.recycle();
        initPaint();
        initText();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isLeft) {
            startX = (getWidth() / textCount) + (slashAngle / 2);
            stopX = (getWidth() / textCount) - (slashAngle / 2);
        } else {
            startX = (getWidth() / textCount) - (slashAngle / 2);
            stopX = (getWidth() / textCount) + (slashAngle / 2);
        }
        ...
        pathLeft.moveTo(0, 0);
        if (isLeft) {
            pathLeft.lineTo(stopX + (slashAngle), 0);
        } else {
            pathLeft.lineTo(stopX - (slashAngle), 0);
        }
        ...
        if (isLeft) {
            pathRight.moveTo(stopX + (slashAngle), 0);
        } else {
            pathRight.moveTo(stopX - (slashAngle), 0);
        }
